...
Letters & Phonics is that child's path. SATPIN Ava sees Cloud 1 letters **S, A, T, P, I, N** and the cloud islands. Assigned Miles sees **S, O, J** and no islands. The next PLAY tile carries a gold ring or a Next up tag. A grown-up pin can add a letter-of-the-day flag when that letter is in the child's work.
...
- **Pin flag.** Navigate to `verify-rsabc smoke 'kid=k01&pin=P&to=%23/trail'`. A tile name includes `Letter of the day`.
...
- **Stray bookmark.** Smoke `kid=k01&play=9`. Screen stays trail. Lucy names the letter that is up.
- **Proof.** For Ava, `localStorage['rsabc.kid']` is `"k01"`. For Miles, `"k24"`.

## Gotchas

- SATPIN Ava's board is S A T P I N. Do not use a Z tile as the path proof.
- `#/letter/9` or `play=9` is a stray on purpose. It must land on Letters & Phonics, not silently start A.
...
- Do not count CSS gold or blue rings as the only proof. The accessible name includes `Letter of the day` and `Starts next.`
