...
- Empty session with `to=%23/pouch` lands on the roster. Seed `kid=k01` first.
...
- Celebrate `Put it on Lucy` is a second entry. Proving only the closet tab does not cover celebrate hand-off. Say so if you skip it.
