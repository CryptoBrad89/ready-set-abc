...
Historical drive used origin `http://127.0.0.1:4174` and pin `rsabc-shell-v36-login-satpin`. Default verify port is now 4173. Current shell pin on origin/main is `rsabc-shell-v40-nudge-home`. This note does not re-drive 4174. Do not drive 8000 or 8080.
...
- Child tap to Home. Ava's Home shows `Welcome back, Ava!`, hash `#/home`, `rsabc.kid` k01, `rsabc.session` child. Not a round. `03-ava-home.png`.
- SATPIN on Home. Level 1 line `S · A · T · P · I · N`. Mission Play letter P. `03-ava-home.png`.
- PIN 1234 on the pad. Tap 1 2 3 4, sheet heading Grown-Ups. `04-pin-gate.png`, `05-grownups-unlocked.png`. Header unlock does not switch session. Ava is still playing.
- Keyboard PIN. Header Grown-Ups, Digit1 then Digit2 Digit3 Digit4. Sheet heading Grown-Ups. Measured on the v36 drive.
- Assigned-letter child. Class tab, Miles work mode Assigned, letters `S O J`. Miles Home shows `#/home`, `Welcome back, Miles!`, `S · O · J`, PLAY `Play letter S`. `08-miles-assigned-home.png`. Later leftovers replaced Cloud 1 copy on this path with Your letters. See `artifacts/home-cloud1/NOTES.md` and `artifacts/home-level1/NOTES.md`.
...
Point at leftover NOTES and PRs 3 to 6. Do not treat these as a new browser drive.
...
- `#/letter/P` with empty session is the roster (`Who is playing?`), not meet. `11-letter-skip-roster.png` is a before shot.

Also shipped on main:

- Roster footer no longer prints `null`. `faces.js` only appends Back when `hasSession()`.
- Roster chrome sets `who: false`, so the who-chip is off on login.
