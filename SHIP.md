...
Shell pin for this drop: **`rsabc-shell-v44-silent-names`**

**What is new in this one:** Letter-choice taps play the kid-voice phoneme recreations (C = “cuh”). Picture taps use ElevenLabs Flicker for the word (C = “Cat!”). Letter names (“P!”, “C!”) stay silent until Bradley records. Tablets already on v43 need Grown-Ups → Device → Get update, then Reload.
...
5. Wait for it to say **Pinned `rsabc-shell-v44-silent-names`**.
...
It should say all **515** files. Chrome sometimes throws files away when a
...
| "Only 66 of 515 files are cached" | Chrome evicted some. Back on Wi-Fi, tap **Set up this device** |
