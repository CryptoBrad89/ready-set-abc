...
Shell pin for this drop: **`rsabc-shell-v45-letter-names`**

**What is new in this one:** Meet beats play the kid-voice letter-name recordings (C = “C!”). Letter-choice taps still play the phoneme recreations (C = “cuh”). Picture taps use ElevenLabs Flicker for the word (C = “Cat!”). Tablets already on v44 need Grown-Ups → Device → Get update, then Reload.
...
5. Wait for it to say **Pinned `rsabc-shell-v45-letter-names`**.
...
It should say all **541** files. Chrome sometimes throws files away when a
...
| "Only 66 of 541 files are cached" | Chrome evicted some. Back on Wi-Fi, tap **Set up this device** |
